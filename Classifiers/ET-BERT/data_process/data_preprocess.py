...
def get_feature_flow(label_pcap):
    packets = scapy.rdpcap(label_pcap)
    packet_length = []
    packet_time = []
    packet_direction = []
...
def generation(pcap_path, samples, dataset_save_path="C:\\Users\\code\\Traffic-Classification-Research\\Datasets\\NetFlow-QUIC1", dataset_level="flow"):
    ...
    for key in tqdm.tqdm(session_pcap_path.keys()):
        ...
        for r_f in r_files:
            if dataset_level == "flow":
                packet_length, packet_time, packet_direction = get_feature_flow(r_f)
            elif dataset_level == "packet":
...
if __name__ == '__main__':
    pcap_path, samples = "C:\\Users\\code\\Traffic-Classification-Research\\Datasets\\NetFlow-QUIC1\\splitcap", 100
    ...
    print("First few elements of X:")
    for feature_data in X[10:12]:
    	print(feature_data)  # Adjust the number as needed
