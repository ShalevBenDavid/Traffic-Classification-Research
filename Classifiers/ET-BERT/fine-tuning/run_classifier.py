...
class Classifier(nn.Module):
    def __init__(self, args):
        ...
        self.embedding = WordSinusoidalposEmbedding(args, len(args.tokenizer.vocab))
...
def tsv_to_dataframe(path):
    try:
        df = pd.read_csv(path, sep='\t', lineterminator='\r', quoting=3)
        df['label'] = df['label'].str.strip()
        df['text_a'] = df['text_a'].str.strip()
...
def read_dataset(args, path):
    # Use the tsv_to_dataframe function to read and preprocess the TSV file
    ...
    for index, row in df.iterrows():
        ...
        tgt = int(row['label'])  # Accessing 'label' column
        text_a = row['text_a']   # Accessing 'text_a' column

        if args.soft_targets and "logits" in df.columns:
            soft_tgt = [float(value) for value in str(row['logits']).split(" ")]

        if "text_b" not in df.columns:  # Sentence classification.
...
def evaluate(args, dataset, print_confusion_matrix=False):
    ...
    if print_confusion_matrix:
        ...
        with open("C:\\Users\\code\\Traffic-Classification-Research\\Datasets\\NetFlow-QUIC1\\results.txt", 'a') as f:
...
def main():
    ...
    args.config_path = 'C:\\Users\\code\\Traffic-Classification-Research\\ET-BERT\\models\\bert\\base_config.json'
    ...
    for epoch in tqdm.tqdm(range(1, args.epochs_num + 1)):
        ...
        if result[0] > best_result:
            ...
            model_dir = os.path.dirname(args.output_model_path)
